...
from mozgatas_data.mozgatas1_data import Mozgatas1B_data
from mozgatas_data.mozgatas1_draw import Mozgatas1B_View
...
class App(PLC_ViewA):
    ...
    def __init__(self, screenName=None, baseName=None, className='Tk', useTk=True, sync=False, use=None):
        ...
        self.title('Mozgatás 1B')
        # noinspection SpellCheckingInspection
        self.name_label.config(text='Mozgatás 1B')

        self.indicator = Mozgatas1B_View(self.process_frame)
        self.indicator.pack()

        self.plc_data = Mozgatas1B_data(self.ip_select.ip_address.get(), self.plc_rack, self.plc_slot)
    ...
    def controls_refresh(self):
        # 1 1 1
        if self.plc_data.start and self.plc_data.stop and self.plc_data.uzem:
            self.indicator.controls_change_color(start_color='green', stop_color='red', factory_color='green')
        # 0 1 1
        elif not self.plc_data.start and self.plc_data.stop and self.plc_data.uzem:
            self.indicator.controls_change_color(start_color='gray', stop_color='red', factory_color='green')
        # 1 0 1
        elif self.plc_data.start and not self.plc_data.stop and self.plc_data.uzem:
            self.indicator.controls_change_color(start_color='green', stop_color='gray', factory_color='green')
        # 0 0 1
        elif not self.plc_data.start and not self.plc_data.stop and self.plc_data.uzem:
            self.indicator.controls_change_color(start_color='gray', stop_color='gray', factory_color='green')
        # 1 1 0
        elif self.plc_data.start and self.plc_data.stop and not self.plc_data.uzem:
            self.indicator.controls_change_color(start_color='green', stop_color='red', factory_color='gray')
        # 0 1 0
        elif not self.plc_data.start and self.plc_data.stop and not self.plc_data.uzem:
            self.indicator.controls_change_color(start_color='gray', stop_color='red', factory_color='gray')
        # 1 0 0
        elif self.plc_data.start and not self.plc_data.stop and not self.plc_data.uzem:
            self.indicator.controls_change_color(start_color='green', stop_color='gray', factory_color='gray')
        # 0 0 0
        else:
            self.indicator.controls_change_color(start_color='gray', stop_color='gray', factory_color='gray')

    def sensor_refresh(self):
        # 1 1 1
        if self.plc_data.e0 and self.plc_data.e1 and self.plc_data.e2:
            self.indicator.sensor_change_color(sensor0_color='green', sensor1_color='green', sensor2_color='green')
        # 0 1 1
        elif not self.plc_data.e0 and self.plc_data.e1 and self.plc_data.e2:
            self.indicator.sensor_change_color(sensor0_color='gray', sensor1_color='green', sensor2_color='green')
        # 1 0 1
        elif self.plc_data.e0 and not self.plc_data.e1 and self.plc_data.e2:
            self.indicator.sensor_change_color(sensor0_color='green', sensor1_color='gray', sensor2_color='green')
        # 0 0 1
        elif not self.plc_data.e0 and not self.plc_data.e1 and self.plc_data.e2:
            self.indicator.sensor_change_color(sensor0_color='gray', sensor1_color='gray', sensor2_color='green')
        # 1 1 0
        elif self.plc_data.e0 and self.plc_data.e1 and not self.plc_data.e2:
            self.indicator.sensor_change_color(sensor0_color='green', sensor1_color='green', sensor2_color='gray')
        # 0 1 0
        elif not self.plc_data.e0 and self.plc_data.e1 and not self.plc_data.e2:
            self.indicator.sensor_change_color(sensor0_color='gray', sensor1_color='green', sensor2_color='gray')
        # 1 0 0
        elif self.plc_data.e0 and not self.plc_data.e1 and not self.plc_data.e2:
    ...
    def indicator_refresh(self):
        # 1 1 1
        if self.plc_data.m_fel and self.plc_data.m_fog and self.plc_data.m_le:
            self.indicator.indicator_change_color(motor_up_color='red', motor_stack_color='red', motor_down_color='red')
        # 0 1 1
        elif not self.plc_data.m_fel and self.plc_data.m_fog and self.plc_data.m_le:
            self.indicator.indicator_change_color(motor_up_color='gray', motor_stack_color='red', motor_down_color='red')
        # 1 0 1
        elif self.plc_data.m_fel and not self.plc_data.m_fog and self.plc_data.m_le:
            self.indicator.indicator_change_color(motor_up_color='gray', motor_stack_color='red', motor_down_color='gray')
        # 0 0 1
        elif not self.plc_data.m_fel and not self.plc_data.m_fog and self.plc_data.m_le:
            self.indicator.indicator_change_color(motor_up_color='gray', motor_stack_color='gray', motor_down_color='red')
        # 1 1 0
        elif self.plc_data.m_fel and self.plc_data.m_fog and not self.plc_data.m_le:
            self.indicator.indicator_change_color(motor_up_color='red', motor_stack_color='red', motor_down_color='gray')
        # 0 1 0
        elif not self.plc_data.m_fel and self.plc_data.m_fog and not self.plc_data.m_le:
            self.indicator.indicator_change_color(motor_up_color='gray', motor_stack_color='red', motor_down_color='gray')
        # 1 0 0
        elif self.plc_data.m_fel and not self.plc_data.m_fog and not self.plc_data.m_le:
