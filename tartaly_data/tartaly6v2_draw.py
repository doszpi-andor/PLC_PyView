...
from tartaly_data.tartaly6v2_data import Tartaly6v2_Address
...
class Tartaly6v2_View(TankCanvas, ValveCanvas, PipeCanvas, SensorCanvas, AnalogCanvas, IndicatorCanvas):
    ...
    INDICATOR_T2_X_POSITION = T2_TANK_X_POSITION - INDICATOR_WIDTH - 60
    INDICATOR_T3_X_POSITION = T3_TANK_X_POSITION - INDICATOR_WIDTH - 60
    ...
    INDICATOR_T2_T3_ROW2_Y_POSITION = T2_T3_BOTTOM_VALVE_Y_POSITION
    INDICATOR_T2_T3_ROW1_Y_POSITION = INDICATOR_T2_T3_ROW2_Y_POSITION - INDICATOR_WIDTH - 40
    ...
    def __operator_interface_drawing(self):
        self.create_square_indicator(self.INDICATOR_COLUMN1_X_POSITION,
                                     ...
                                     name='Start [%s]' % Tartaly6v2_Address.START, color=self.__start_color)
        self.create_square_indicator(self.INDICATOR_COLUMN1_X_POSITION,
                                     self.INDICATOR_ROW2_Y_POSITION,
                                     name='Stop [%s]' % Tartaly6v2_Address.STOP, color=self.__stop_color)
        self.create_circle_indicator(self.INDICATOR_COLUMN1_X_POSITION,
                                     self.INDICATOR_ROW3_Y_POSITION,
                                     name='Üzem [%s]' % Tartaly6v2_Address.UZEM, color=self.__factory_color)
    ...
    def __tank1_drawing(self):
        self.create_valve(x_position=self.T1_TANK_X_POSITION + self.TANK_WIDTH // 2 - self.VALVE_WIDTH // 2,
                          ...
                          name='T1_Tolt [%s]' % Tartaly6v2_Address.T1_TOLT, color=self.__tank1_top_valve_color)
        ...
        self.create_sensor(x_position=self.T1_TANK_X_POSITION,
                           ...
                           name='T1_Teli\n[%s]' % Tartaly6v2_Address.T1_TELI, color=self.__tank1_sensor_color)
    ...
    def __tank2_drawing(self):
        self.create_valve(x_position=self.T2_TOP_VALVE_X_POSITION,
                          ...
                          name='T2_Tolt [%s]' % Tartaly6v2_Address.T2_TOLT, color=self.__tank2_top_valve_color)
        self.create_valve(x_position=self.T2_ADD_VALVE_X_POSITION,
                          y_position=self.T2_T3_TOP_VALVE_Y_POSITION,
                          name='T2_Adalek [%s]' % Tartaly6v2_Address.T2_ADALEK, color=self.__tank2_add_valve_color)
        ...
        self.create_valve(x_position=self.T2_TANK_X_POSITION + self.TANK_WIDTH // 2 - self.VALVE_WIDTH // 2,
                          ...
                          name='T2_Urit [%s]' % Tartaly6v2_Address.T2_URIT, color=self.__tank2_bottom_valve_color)

    def __tank2_indicator_draving(self):
        self.create_circle_indicator(x_position=self.INDICATOR_T2_X_POSITION,
                                     y_position=self.INDICATOR_T2_T3_ROW1_Y_POSITION,
                                     name='Keverek\nKesz_T2\n[%s]' % Tartaly6v2_Address.T2_KEVEREK_KEZS,
                                     color=self.__tank2_completed_color)
        self.create_square_indicator(x_position=self.INDICATOR_T2_X_POSITION,
                                     y_position=self.INDICATOR_T2_T3_ROW2_Y_POSITION,
                                     name='Start\nUrit_T2\n[%s]' % Tartaly6v2_Address.T2_START_URIT,
                                     color=self.__tank2_dump_color)

    def __tank3_drawing(self):
        self.create_valve(x_position=self.T3_TOP_VALVE_X_POSITION,
                          y_position=self.T2_T3_TOP_VALVE_Y_POSITION,
                          name='T3_Tolt [%s]' % Tartaly6v2_Address.T3_TOLT, color=self.__tank3_top_valve_color)
        self.create_valve(x_position=self.T3_ADD_VALVE_X_POSITION,
                          y_position=self.T2_T3_TOP_VALVE_Y_POSITION,
                          name='T3_Adalek [%s]' % Tartaly6v2_Address.T2_ADALEK, color=self.__tank3_add_valve_color)
        ...
        self.__tank3_analog_id = self.create_analog(x_position=self.T3_TANK_X_POSITION,
                                                    ...
                                                    activ_level_print=True, active_color='red',
                                                    name='T3_Szint [%s]\n' % Tartaly6v2_Address.T3_SZINT)
        self.create_valve(x_position=self.T3_TANK_X_POSITION + self.TANK_WIDTH // 2 - self.VALVE_WIDTH // 2,
                          y_position=self.T2_T3_BOTTOM_VALVE_Y_POSITION,
                          name='T3_Urit [%s]' % Tartaly6v2_Address.T3_URIT, color=self.__tank3_bottom_valve_color)

    def __tank3_indicator_draving(self):
