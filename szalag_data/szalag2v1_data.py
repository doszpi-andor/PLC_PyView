...
class Szalag2v1_Address(PLC_Address):
...
class Szalag2v1_data(PLC_data):
    ...
    def __init__(self, ip, rack, slot) -> None:
        super().__init__(Szalag2v1_Address(), ip, rack, slot)
    ...
    def read_data(self) -> None:
        ...
        self.__s1 = self.get_bit_tag_page(Szalag2v1_Address.S1)
        self.__s2 = self.get_bit_tag_page(Szalag2v1_Address.S2)
        self.__kp1 = self.get_bit_tag_page(Szalag2v1_Address.KP1)
        self.__kp2 = self.get_bit_tag_page(Szalag2v1_Address.KP2)
        self.__ks = self.get_bit_tag_page(Szalag2v1_Address.KS)
        self.__start = self.get_bit_tag_page(Szalag2v1_Address.START)
        self.__stop = self.get_bit_tag_page(Szalag2v1_Address.STOP)

        self.__m1 = self.get_bit_tag_page(Szalag2v1_Address.M1)
        self.__m2 = self.get_bit_tag_page(Szalag2v1_Address.M2)
        self.__uzem = self.get_bit_tag_page(Szalag2v1_Address.UZEM)
        self.__hiba = self.get_bit_tag_page(Szalag2v1_Address.HIBA)
    ...
    def ks_is_changed(self):
        if self.__ks != self.__ks_old:
